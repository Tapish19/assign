...
<section id="about" class="px-4 lg:px-[60px] xl:px-[100px] py-16">
  <div class="max-w-[1240px] mx-auto grid md:grid-cols-3 gap-6">
    ...
    <div class="bg-[#F5F7FB] p-6 rounded-3xl"><img src="/images/ce_certificate.svg" class="mb-3" alt="">CE Marked Quality</div>
  </div>
</section>
...
<footer class="bg-[#111827] text-white px-4 lg:px-[60px] xl:px-[100px] py-10">
  <div class="max-w-[1240px] mx-auto flex flex-col md:flex-row justify-between gap-6">
    <div><img src="/images/footer_mobile_logo.svg" alt="footer" class="mb-3"><p class="text-sm text-slate-300">Built for modern infrastructure projects across India.</p></div>
    <div class="text-sm text-slate-300">support@euroflex.co<br>+91 90000 10000</div>
  </div>
</footer>
`;
...
faqData.forEach(({ q, a }, idx) => {
  ...
  item.className = 'border border-[#E1E3E8] rounded-2xl p-5 bg-white';
  item.innerHTML = `<button class="faq-btn w-full text-left flex justify-between items-center font-semibold">${q}<img src="${idx === 0 ? '/images/accordian-open.svg' : '/images/accordian-close.svg'}" class="faq-icon" alt="toggle"></button><p class="faq-answer ${idx === 0 ? '' : 'hidden'} text-[#4D545C] mt-3 leading-7">${a}</p>`;
...
faqList.querySelectorAll('.faq-btn').forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    icon.src = open ? '/images/accordian-close.svg' : '/images/accordian-open.svg';
  });
});
